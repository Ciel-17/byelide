  ...
  <div class="app-navigator">
    ...
    <div class="app-navigator-link-wrapper">
      <router-link
        ...
        v-for="route in linkItems"
        :key="route.value"
        :to="`/app/${route.value}`"
        :style="activeLink.includes(route.value) && { background: route.bg }"
      >
        <div
          :style="{
            lineHeight: 0.7,
            color: activeLink.includes(route.value) ? route.color : 'var(--color-gray-700)'
          }"
        >
          <Icon :type="route.value" :active="activeLink.includes(route.value)" />
        </div>
        ...
        <div
          class="item-border"
          :style="activeLink.includes(route.value) ? { background: route.borderColor } : {}"
        ></div>
      </router-link>
    </div>
...
import { useEnvStore } from '@/stores/debug.ts'
